...
function convertToHTML(swaggerJSON){
    ...
    html += '<div class="coverHeadings"><h1>Introduction简介</h1></div>';
    
    html += headerSummary(swaggerJSON);
 
    html += "<div style='page-break-after:always'></div>";
    html += "<div style='page-break-after:always'><div class='footer'>Generated by lichenyu on: " +genDate +"</div>";
    ...
    if(typeof(swaggerJSON.definitions) !=='undefined')
    {
        ...
        for(var dfn in swaggerJSON.definitions) {
            ...
            html += "<h3>1." +sub1Counter + ". " + dfn + (swaggerJSON.definitions[dfn].description != null ? " " + swaggerJSON.definitions[dfn].description : "" ) + "</h3>";
            html += "<hr />";
...
function tableOfContents(swaggerJSON)
{
    ...
    if(main2Counter===2)//there are definitions
    {
            ...
            htmlTOC += '<div class="coverHeadings"><h1>Table of contents目录</h1></div>';
            htmlTOC += '<h2>' +main1Counter+'. Definitions</h2>';
            sub1Counter=main1Counter;
            for(var dfn in swaggerJSON.definitions) {
                // eg: Product (uber)
                htmlTOC += '<div class="div-container-margin">'; // definitions start
                htmlTOC += "<span><b>"+main1Counter+"." + sub1Counter +".</b> " + dfn + (swaggerJSON.definitions[dfn].description != null ? " " + swaggerJSON.definitions[dfn].description : "" )+ "</span>";
                htmlTOC += "<br><br>";
...
function pathsTableContents(swaggerJSON, mainCounter)
{
    ...
    for(var path in swaggerJSON.paths){
     ...
     for(var action in swaggerJSON.paths[path]){
            ...
            {
                case "get":
                    html+='<span><b>' +mainCounter +'.' + sub3Counter +'.</b> ' +'get</span>&nbsp;&nbsp;'+path+(typeof(swaggerJSON.paths[path][action].summary) !== "undefined"?(" " + swaggerJSON.paths[path][action].summary):"");
                    break; 
                case "post":
                    html+='<span><b>' +mainCounter +'.' + sub3Counter +'.</b> ' +'post</span>&nbsp;&nbsp;'+path+(typeof(swaggerJSON.paths[path][action].summary) !== "undefined"?(" " + swaggerJSON.paths[path][action].summary):"");
                    break;
                case "put":
                    html+='<span><b>' +mainCounter +'.' + sub3Counter +'.</b> ' +'put</span>&nbsp;&nbsp;'+path+(typeof(swaggerJSON.paths[path][action].summary) !== "undefined"?(" " + swaggerJSON.paths[path][action].summary):"");
                    break;
                case "delete":
                    html+='<span><b>' +mainCounter +'.' + sub3Counter +'.</b> ' +'delete</span>&nbsp;&nbsp;'+path+(typeof(swaggerJSON.paths[path][action].summary) !== "undefined"?(" " + swaggerJSON.paths[path][action].summary):"");
                    break;
            }
            html += "<br><br>";
            sub3Counter++;
...
function headerSummary(swaggerJSON){
    ...
    html+="<div><h1>"+ swaggerJSON.info.title +"</h1></div>";
     if(swaggerJSON.info.description != null) {
        ...
        html+= "<div>"+ "api在线文档地址为http://{ip}:{port}/{basePath}/swagger-ui.html" +"</div>";
    }
    html += "<br />";
